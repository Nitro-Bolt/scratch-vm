    ...
    descendInput (block) {
        ...
        case InputOpcode.CONSTANT:
            if (block.isAlwaysType(InputType.NUMBER)) {
                ...
                if (typeof node.value !== 'object') throw new Error(`JS: '${block.type}' type constant had ${typeof node.value} type value. Expected object.`);
                return `(${Cast.toString(node.value)})`;
            } else if (block.isAlwaysType(InputType.ARRAY)) {
                if (!Array.isArray(node.value)) throw new Error(`JS: '${block.type}' type constant had ${typeof node.value} type value. Expected array.`);
                return `(${Cast.toString(node.value)})`;
