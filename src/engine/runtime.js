...
class Runtime extends EventEmitter {
    constructor () {
        ...
        Object.defineProperty(this, '_lastStepTime', {
            get: () => this.frameLoop._lastStepTime
        });
    ...
    _reorderExtensionPrimitive (extensionIndex, reorderIndex) {
        ...
        this._blockInfo.splice(
            reorderIndex,
            0,
            this._blockInfo.splice(extensionIndex, 1)[0]
        );
    ...
    _removeExtensionPrimitive (extensionId) {
        const extensionIndex = this._blockInfo.findIndex(
    ...
    _convertBlockForScratchBlocks (blockInfo, categoryInfo) {
        ...
        case BlockType.OBJECT:
            ...
            blockJSON.outputShape =
                    ScratchBlocksConstants.OUTPUT_SHAPE_OBJECT;
            break;
        ...
        const blockXML = `<block type="${xmlEscape(
            extendedOpcode
        )}">${mutation}${inputs}</block>`;
    ...
    _convertButtonForScratchBlocks (buttonInfo, categoryInfo) {
        ...
        if (nativeCallbackKeys.includes(buttonInfo.func)) {
            return {
                info: buttonInfo,
                xml: `<button text="${xmlEscape(
                    buttonText
                )}" callbackKey="${xmlEscape(buttonInfo.func)}"></button>`
            };
        }
    ...
    _convertPlaceholders (context, match, placeholder) {
        ...
        if (argTypeInfo.fieldType === 'field_image') {
            ...
            if (defaultValue !== null && fieldName) {
                context.inputList.push(
                    `<field name="${xmlEscape(fieldName)}">${xmlEscape(
                        defaultValue
                    )}</field>`
                );
            }
    ...
    getBlocksXML (target) {
        return this._blockInfo.map(categoryInfo => {
            ...
            const colorXML = `colour="${xmlEscape(
                color1
            )}" secondaryColour="${xmlEscape(color2)}"`;
    ...
    toggleScript (topBlockId, opts) {
        if (opts?.stackClick && !this.frameLoop.running) {
        ...
        if (!options.hidden) {
            ...
            if (!blockInfo) {
                ...
                blockInfo = {
                    ...
                    color2: '#3aa8a4',
                    color3: '#3aa8a4',
                    menuIconURI: `data:image/svg+xml;,${encodeURIComponent(
                        ICON
                    )}`,
                    blocks: [],
                    customFieldTypes: {},
            ...
            blockInfo.blocks.push({
                ...
                xml:
                    ...
                    ` argumentdefaults="${xmlEscape(
                        JSON.stringify(defaults)
                    )}"` +
                    `${
                        options.return ?
                            ` return="${xmlEscape(
                                options.return.toString()
                            )}"` :
                            ''
                    }` +
                    '></mutation></block>'
    ...
    requestUpdateMonitor (monitor) {
        ...
        if (visible && !this.frameLoop.running) {
            this.frameLoop.start();
        }
